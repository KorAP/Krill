...
/**
 * Create a tokenstream with the first token being the verbatim
 * string.
 * All following tokens are standardtokenized and lowercased.
 */

public class TextPrependedTokenStream extends TokenStream {
    private final CharTermAttribute charTermAttr = this
            .addAttribute(CharTermAttribute.class);
    private final PositionIncrementAttribute posIncrAttr = this
    ...
    public final boolean incrementToken () throws IOException {
        ...
        if (this.scanner == null) {
            this.scanner = new StandardTokenizerImpl(new StringReader(
                    this.verbatim));
        };
        ...
        while (true) {
            ...
            if (scanner.yylength() <= maxTokenLength) {
                posIncrAttr.setPositionIncrement(skippedPositions + 1);
                scanner.getText(charTermAttr);
                charUtils.toLowerCase(charTermAttr.buffer(), 0,
                        charTermAttr.length());
                return true;
            }
            else
