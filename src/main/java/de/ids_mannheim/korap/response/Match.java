...
public class Match extends AbstractDocument {
    ...
    public ObjectNode getSnippetTokens () {
        ...
        if (startContext == -1) {
            startContext = this.getStartPos();
            if (DEBUG)
                log.debug("Set startContext {}", endContext);
        };

        if (endContext == -1) {
            endContext = this.getEndPos() - 1;
            if (DEBUG)
        ...
        if (endContextChar == -1)
            endContextChar = pto.start(ldid, endContext);

        if (DEBUG)
            log.debug("Match is {}/{} - {}/{}",startContext,startContextChar,endContext,endContextChar);

        if (endContextChar == 0 || endContextChar > this.getPrimaryDataLength()) {
        ...
        if (startContext < this.getStartPos()) {
            tokens = json.putArray("left");
            for (i = startContext; i < this.getStartPos(); i++) {
        ...
        if (endContext > this.getEndPos()) {
            tokens = null;
            for (i = this.getEndPos(); i < endContext; i++) {
                offsets = pto.span(ldid,i);
                if (offsets == null) {
                    break;
                };
        ...
        if (this.highlight != null) {
            ...
            int matchOffset = startContext + this.getStartPos();
            for (Highlight highlight : this.highlight) {
                ArrayNode cls = mapper.createArrayNode();                
                cls.add(highlight.number);
                cls.add(highlight.start-matchOffset);
                cls.add(highlight.end-matchOffset);
                classes.add(cls);
            };
        };
